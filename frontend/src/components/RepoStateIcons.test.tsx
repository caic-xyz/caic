...
vi.mock("@solidjs/router", () => ({
  A: (props: { children: JSX.Element; href: string; class?: string; title?: string; "aria-label"?: string }) => (
    <a class={props.class} href={props.href} title={props.title} aria-label={props["aria-label"]}>{props.children}</a>
  ),
}));

it("renders dense markers with an accessible state summary", () => {
  render(() => (
    <RepoStateIcons state={{
      name: "repo",
      branch: "main",
      ahead: 2,
      behind: 1,
...
it("accepts a parent layout request to elide line totals", () => {
  render(() => (
    <RepoStateIcons
      elideDiffStats
      state={{ name: "repo", branch: "main", ahead: 1, behind: 0, changedFiles: 4, added: 17, deleted: 2, uncommittedFiles: 0, conflicts: 0 }}
    />
  ));
...
it("links a state marker to its repository diff", () => {
  render(() => (
    <RepoStateIcons
      ...
      state={{ name: "repo", branch: "main", ahead: 1, behind: 0, changedFiles: 1, added: 1, deleted: 0, uncommittedFiles: 0, conflicts: 0 }}
    />
  ));
