...
lint: lint-go lint-frontend lint-python lint-binaries lint-docs lint-format
...
# Check the formatting of the tracked sources the pre-commit hook formats. The hook
# only looks at staged files, so without this target drift accumulates silently
# until someone happens to edit one of the files. AGENTS.md and its CLAUDE.md
# symlinks are generated by the file-index script, which the hook skips.
lint-format:
	@git ls-files | grep -E '\.(css|html|json|md|mjs|ts|tsx|yaml|yml)$$' | grep -vE '(^|/)(AGENTS|CLAUDE)\.md$$' | xargs -d '\n' pnpm exec prettier --check
