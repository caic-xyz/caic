...
- `README.md`: oauth
- `docs/split-plan.md`: OAuth Package Split
- `dto.go`: Package oauth is an OAuth 2.0 authorization server and client library.
- `dto_client.go`: Client-side OAuth 2.0 DTOs.
- `dto_server.go`: Server-side OAuth 2.0 DTOs: types consumed by the authorization server,
- `oauthclient/client.go`: Authorization-code client helpers and forge provider configurations.
- `oauthclient/client_test.go`: Tests for OAuth authorization-code client helpers.
- `oauthclient/forge_github.go`: GitHub OAuth client provider configuration.
- `oauthclient/forge_gitlab.go`: GitLab OAuth client provider configuration.
