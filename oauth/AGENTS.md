...
**Planned**: this package will be extracted to a standalone Go module
(`github.com/caic-xyz/oauth`). It will serve caic and at least one
additional client project. All design decisions must consider the
standalone context — no caic-specific types, no backend dependencies,
stdlib only.
...
- `oauthclient/provider.go`: Provider (GitHub, GitLab) configurations with userinfo fetching.
...
- `oauthserver/storage.go`: OAuth durable authorization state storage.
