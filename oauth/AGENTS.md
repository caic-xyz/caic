...
- `README.md`: oauth
- `client.go`: OAuth authorization-code client helpers.
- `client_test.go`: Tests for OAuth authorization-code client helpers.
- `dpopjti.go`: DPoP proof replay prevention via a bounded jti seen-set (RFC 9449 §11.1).
- `dpopnonce.go`: DPoP nonce management per RFC 9449 §8.
- `dpopnonce_test.go`: Tests for DPoP nonce management.
- `dpopproof.go`: DPoP sender-constrained token proof-of-possession (RFC 9449).
...
- `server.go`: OAuth authorization-server HTTP handlers.
- `server_test.go`: Tests for generic OAuth authorization-server HTTP handlers.
- `state.go`: OAuth CSRF state helpers.
- `storage.go`: OAuth durable authorization state storage.
- `storage_test.go`: Tests for OAuth durable authorization state storage.
- `token.go`: OAuth access-token signing and verification.
- `token_test.go`: Tests for OAuth access-token signing and verification.
<!-- END FILE INDEX -->
