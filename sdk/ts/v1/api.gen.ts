...
export function getPreferences(): Promise<PreferencesResp> {
  return request<PreferencesResp>("GET", "/api/v1/server/preferences");
}
...
export function cloneRepo(req: CloneRepoReq): Promise<Repo> {
  return request<Repo>("POST", "/api/v1/server/repos", req);
}

export function listRepoBranches(repo: string): Promise<RepoBranchesResp> {
  return request<RepoBranchesResp>("GET", `/api/v1/server/repos/branches?repo=${encodeURIComponent(repo)}`);
}
...
export function getTaskCILog(id: string, jobID: string): Promise<CILogResp> {
  return request<CILogResp>("GET", `/api/v1/tasks/${id}/ci-log?jobID=${encodeURIComponent(jobID)}`);
}
...
export function getTaskToolInput(id: string, toolUseID: string): Promise<TaskToolInputResp> {
  return request<TaskToolInputResp>("GET", `/api/v1/tasks/${id}/tool/${toolUseID}`);
}

export function globalTaskEvents(onMessage: (event: TaskListEvent) => void): EventSource {
...
export function webFetch(req: WebFetchReq): Promise<WebFetchResp> {
  return request<WebFetchResp>("POST", "/api/v1/web/fetch", req);
}
