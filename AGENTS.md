...
- `oauth/client.go`: OAuth authorization-code client helpers.
- `oauth/dpopnonce.go`: DPoP nonce management per RFC 9449 §8.
- `oauth/dpopproof.go`: DPoP sender-constrained token proof-of-possession (RFC 9449).
- `oauth/dto.go`: Package oauth is an OAuth 2.0 authorization server and client library.
- `oauth/server.go`: OAuth authorization-server HTTP handlers.
- `oauth/server_test.go`: Tests for generic OAuth authorization-server HTTP handlers.
- `oauth/state.go`: OAuth CSRF state helpers.
- `oauth/storage.go`: OAuth durable authorization state storage.
- `oauth/storage_test.go`: Tests for OAuth durable authorization state storage.
- `oauth/token.go`: OAuth access-token signing and verification.
- `oauth/token_test.go`: Tests for OAuth access-token signing and verification.
...
- `test_android_e2e.py`: Tests for Android E2E runner readiness helpers.
