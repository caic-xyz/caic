...
1. Extract remaining route groups into constructed handler structs with no
   back-reference to concrete `Server`: bot, CI, usage, server config, webhook,
   and voice. Prefer explicit dependencies over function fields unless a
   callback is the clearest boundary.
