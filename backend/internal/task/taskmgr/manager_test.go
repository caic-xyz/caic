...
func TestManager(t *testing.T) {
	...
	t.Run("SendInput", func(t *testing.T) {
		...
		t.Run("valid_reconnects_before_answering_restored_ask", func(t *testing.T) {
			...
			path := filepath.Join(logDir, tk.LogFilename())
			header := tk.LogHeader()
			logW, err := (&task.LogStore{LogDir: logDir}).Open(path, &header)
			if err != nil {
				t.Fatal(err)
			}
	...
	t.Run("LoadMessagesOnDemand", func(t *testing.T) {
		...
		t.Run("uses_header_resolver", func(t *testing.T) {
			...
			entry := newTestPurgedEntry(t, tk, &task.Result{State: task.StatePurged}, logs[0])
	...
	t.Run("Revive", func(t *testing.T) {
		...
		t.Run("error_failure_closes_done", func(t *testing.T) {
			...
			tk.SetRuntimeConnectionInfo(runtime.NewID("test-runtime", "ctr-1"), runtime.ConnectionTarget{SSHHost: "ctr-1"}, "", "", 0)
			tk.SetState(task.StateStopped)
			path := filepath.Join(m.Logs.LogDir, tk.LogFilename())
			header := tk.LogHeader()
			log, err := m.Logs.Open(path, &header)
			if err != nil {
				t.Fatal(err)
			}
...
`)...)
		...
		t.Run("missing_local_log_refuses_live_reconnect", func(t *testing.T) {
			...
			if adopted[0].LogPath() != "" {
				t.Fatalf("replacement LogPath = %q, want empty", adopted[0].LogPath())
			}
		})
