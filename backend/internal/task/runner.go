...
// ContainerBackend abstracts runtime lifecycle operations for testability.
type ContainerBackend interface {
	// Launch starts the runtime instance and writes connection config.
	// writes SSH config. Does NOT wait for SSH. Repos must have branches set.
	// Returns the instance ID assigned during launch.
	Launch(ctx context.Context, repos []runtime.Repo, labels []string, opts *StartOptions) (runtime.InstanceID, error)
	...
	Connect(ctx context.Context, id runtime.InstanceID, repos []runtime.Repo, opts *StartOptions) (runtime.ConnectionInfo, error)
	Diff(ctx context.Context, repos []runtime.Repo, repoIdx int, args ...string) (string, error)
	Fetch(ctx context.Context, repos []runtime.Repo) error
	// Stop gracefully stops the container without removing it. The container
	// can be restarted later with Revive.
	Stop(ctx context.Context, id runtime.InstanceID) error
	// Purge stops and removes the container identified by name, cleaning up
	// SSH config and git remotes for the given repos.
	Purge(ctx context.Context, id runtime.InstanceID, repos []runtime.Repo) error
	...
	Revive(ctx context.Context, id runtime.InstanceID, repos []runtime.Repo) error
	...
	Fork(ctx context.Context, id runtime.InstanceID, repos []runtime.Repo, opts *ForkOptions) (runtime.InstanceID, []runtime.Repo, error)
	// VNCPort returns the Docker host port mapped to the container's VNC
	// port (5901). Returns 0 when the container has no display.
	VNCPort(ctx context.Context, id runtime.InstanceID) int
...
type ForkOptions struct {
	ExtraRepos []runtime.Repo // Additional repos to map into the fork beyond the source's repos.
...
func (r *Runner) Cleanup(ctx context.Context, t *Task, reason State) Result {
	...
	if name != "" && r.Container != nil {
		...
		pStart := time.Now()
		if err := r.PurgeContainer(ctx, name, primaryBranch, t.ExtraRuntimeRepos()); err != nil {
...
func (r *Runner) ReviveTask(ctx context.Context, t *Task) (*SessionHandle, error) {
	...
	if err := r.Container.Revive(ctx, runtime.InstanceID(containerName), repos); err != nil {
	...
	if ds := r.BranchDiffStat(ctx, t.RuntimeRepos()); len(ds) > 0 {
...
func (r *Runner) SyncToOrigin(ctx context.Context, repos []runtime.Repo, container string, force bool) (agent.DiffStat, []SafetyIssue, error) {
	r.initDefaults()
	if r.Dir == "" {
...
// task title.
func (r *Runner) SyncToDefault(ctx context.Context, repos []runtime.Repo, container, message string) (agent.DiffStat, []SafetyIssue, error) {
	r.initDefaults()
	if r.Dir == "" {
		return nil, nil, errors.New("sync is not supported for no-repo tasks")
	}
...
func (r *Runner) DiffContent(ctx context.Context, repos []runtime.Repo, path string) (string, error) {
	r.initDefaults()
	if r.Dir == "" {
...
func (r *Runner) PurgeContainer(ctx context.Context, containerName, branch string, extraRepos []runtime.Repo) error {
	r.initDefaults()
	ctx, cancel := context.WithTimeout(context.WithoutCancel(ctx), r.GitTimeout)
	defer cancel()
	var repos []runtime.Repo
	if r.Dir != "" {
		rr := runtime.Repo{HostPath: r.Dir, Branch: branch}
		if r.RepoName != "" {
			rr.MountPath = "~/src/" + r.RepoName
		}
		repos = append([]runtime.Repo{rr}, extraRepos...)
	}
	return r.Container.Purge(ctx, runtime.InstanceID(containerName), repos)
}
...
func (r *Runner) BranchDiffStat(ctx context.Context, repos []runtime.Repo) agent.DiffStat {
	r.initDefaults()
...
func (r *Runner) setup(ctx context.Context, t *Task, labels []string, resolvedGitHubToken string) (setupResult, error) {
	...
	eg.Go(func() error {
		...
		id, err := r.Container.Launch(egCtx, repos, labels, opts)
		if err != nil {
	...
	conn, err := r.Container.Connect(startCtx, runtime.InstanceID(containerName), repos, opts)
	if err != nil {
...
func (r *Runner) fetchDiffStatBranch(ctx context.Context, t *Task, repos []runtime.Repo) {
	fetchCtx, fetchCancel := context.WithTimeout(context.WithoutCancel(ctx), r.GitTimeout)
	defer fetchCancel()
	r.branchMu.Lock()
	defer r.branchMu.Unlock()
...
func (r *Runner) diffStat(ctx context.Context, repos []runtime.Repo) agent.DiffStat {
	if r.Dir == "" {
		return nil
	}
	var result agent.DiffStat
