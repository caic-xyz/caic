...
func (r *Runner) ReviveTask(ctx context.Context, t *Task) (*SessionHandle, error) {
	...
	tlog.Debug("container", "msg", "calling container.Revive", "repos_count", len(repos))
	if err := r.Container.Revive(ctx, t.Container, repos); err != nil {
		tlog.Error("container", "msg", "container.Revive failed", "err", err)
		t.SetState(StateFailed)
		return nil, fmt.Errorf("revive container: %w", err)
	}
	tlog.Debug("container", "msg", "container.Revive succeeded", "container", t.Container)
...
func (r *Runner) ForkTask(ctx context.Context, source, fork *Task, forkOpts *ForkOptions) (*SessionHandle, error) {
	...
	tlog.Debug("container", "msg", "calling container.Fork", "source", source.Container, "harness", forkOpts.Harness, "tailscale", forkOpts.Tailscale, "usb", forkOpts.USB, "display", forkOpts.Display)
	...
	if err != nil {
		tlog.Error("container", "msg", "container.Fork failed", "source", source.Container, "err", err)
		fork.SetState(StateFailed)
		return nil, fmt.Errorf("fork container: %w", err)
	}
	tlog.Debug("container", "msg", "container.Fork succeeded", "source", source.Container, "fork", forkName)
...
func (r *Runner) setup(ctx context.Context, t *Task, labels []string) (setupResult, error) {
	...
	r.log.Debug("container", "msg", "provisioning phase A: launching container and creating branch", "harness", opts.Harness, "tailscale", opts.Tailscale, "usb", opts.USB, "display", opts.Display, "repos_count", len(repos))
	eg, egCtx := errgroup.WithContext(startCtx)
	eg.Go(func() error {
		r.log.Debug("container", "msg", "calling container.Launch", "branch", primaryBranch)
		name, err := r.Container.Launch(egCtx, repos, labels, opts)
		if err != nil {
			r.log.Error("container", "msg", "container.Launch failed", "branch", primaryBranch, "err", err)
			return err
		}
		r.log.Debug("container", "msg", "container.Launch succeeded", "container", name)
		containerName = name
		return nil
	})
	if r.Dir != "" {
		eg.Go(func() error {
			r.log.Debug("container", "msg", "fetching and creating branch", "branch", primaryBranch)
			err := r.fetchAndCreateBranch(egCtx, t, primaryBranch)
			if err != nil {
				r.log.Error("container", "msg", "fetchAndCreateBranch failed", "branch", primaryBranch, "err", err)
			} else {
				r.log.Debug("container", "msg", "fetchAndCreateBranch succeeded", "branch", primaryBranch)
			}
			return err
		})
	}
	r.log.Debug("container", "msg", "waiting for phase A errgroup")
	if err := eg.Wait(); err != nil {
		return setupResult{}, err
	}
	r.log.Debug("container", "msg", "phase A complete", "container", containerName)

	// Phase B: wait for SSH + push (branch now exists locally).
	r.log.Debug("container", "msg", "provisioning phase B: connecting via SSH", "container", containerName, "repos_count", len(repos))
	tailscaleFQDN, err := r.Container.Connect(startCtx, containerName, repos, opts)
	if err != nil {
		r.log.Error("container", "msg", "container.Connect failed", "container", containerName, "err", err)
		return setupResult{}, fmt.Errorf("start container: %w", err)
	}
	r.log.Debug("container", "msg", "container.Connect succeeded", "container", containerName, "fqdn", tailscaleFQDN)
