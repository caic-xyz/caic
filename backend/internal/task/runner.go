...
func (r *Runner) Start(ctx context.Context, t *Task, resolvedGitHubToken string) (*SessionHandle, error) {
	...
	r.Workspace.log.Info("executor", "msg", "ready", "br", primaryBranch, "instance", sr.InstanceID, "dur", time.Since(tStart))
...
func (r *Runner) setup(ctx context.Context, t *Task, metadata runtime.Metadata, resolvedGitHubToken string) (setupResult, error) {
	...
	r.Workspace.log.Debug("executor", "msg", "provisioning phase A: launching instance and creating branch", "harness", opts.Harness, "tailscale", opts.Tailscale, "usb", opts.USB, "display", opts.Display, "sudo", opts.Sudo, "repos_count", len(repos))
	eg, egCtx := errgroup.WithContext(startCtx)
	eg.Go(func() error {
		defer trace.StartRegion(egCtx, "instance-launch").End()
		r.Workspace.log.Debug("executor", "msg", "calling instance.Launch", "branch", primaryBranch)
		id, err := r.runtime().Launch(egCtx, repos, opts)
		if err != nil {
			r.Workspace.log.Error("executor", "msg", "instance.Launch failed", "branch", primaryBranch, "err", err)
			return err
		}
		r.Workspace.log.Debug("executor", "msg", "instance.Launch succeeded", "instance", id)
		instanceID = id
		return nil
	})
	if r.Workspace.Dir != "" {
		eg.Go(func() error {
			...
			r.Workspace.log.Debug("executor", "msg", "fetching and creating branch", "branch", primaryBranch)
			err := r.Workspace.fetchAndCreateBranch(egCtx, t, primaryBranch)
			if err != nil {
				r.Workspace.log.Error("executor", "msg", "fetchAndCreateBranch failed", "branch", primaryBranch, "err", err)
			} else {
				r.Workspace.log.Debug("executor", "msg", "fetchAndCreateBranch succeeded", "branch", primaryBranch)
			}
			return err
		})
	}
	r.Workspace.log.Debug("executor", "msg", "waiting for phase A errgroup")
	if err := eg.Wait(); err != nil {
		return setupResult{}, err
	}
	r.Workspace.log.Debug("executor", "msg", "phase A complete", "instance", instanceID)

	// Phase B: wait for runtime connection + push (branch now exists locally).
	r.Workspace.log.Debug("executor", "msg", "provisioning phase B: connecting to instance", "instance", instanceID)
	conn, err := r.runtime().Connect(startCtx, instanceID, opts)
	if err != nil {
		r.Workspace.log.Error("executor", "msg", "instance.Connect failed", "instance", instanceID, "err", err)
		return setupResult{}, fmt.Errorf("start instance: %w", err)
	}
	r.Workspace.log.Info("executor", "msg", "started", "br", primaryBranch, "dur", time.Since(tContainer), "instance", instanceID, "fqdn", conn.TailscaleFQDN)
	return setupResult{
