// RepoWorkspace owns repo-level branch allocation, git sync, and runtime diff operations.
...
func (w *RepoWorkspace) AllocateBranch(ctx context.Context) (string, error) {
	...
	w.branchMu.Lock()
	defer w.branchMu.Unlock()
	return w.allocateBranchLocked(ctx, &Task{})
}
...
func (w *RepoWorkspace) SyncToOrigin(ctx context.Context, t *Task, force bool) (agent.DiffStat, []SafetyIssue, error) {
	w.initDefaults()
	if w.Dir == "" {
...
// task title.
func (w *RepoWorkspace) SyncToDefault(ctx context.Context, t *Task, message string) (agent.DiffStat, []SafetyIssue, error) {
	w.initDefaults()
	if w.Dir == "" {
		return nil, nil, errors.New("sync is not supported for no-repo tasks")
	}
...
func (w *RepoWorkspace) DiffContent(ctx context.Context, t *Task, path string) (string, error) {
	w.initDefaults()
	if w.Dir == "" {
...
func (w *RepoWorkspace) BranchDiffStat(ctx context.Context, t *Task) agent.DiffStat {
	w.initDefaults()
...
func (w *RepoWorkspace) taskRuntime(t *Task) (runtime.InstanceID, []runtime.Repo, error) {
	if t == nil {
...
func (w *RepoWorkspace) initDefaults() {
	w.initOnce.Do(func() {
		if w.GitTimeout == 0 {
			w.GitTimeout = time.Minute
		}
		repoName := filepath.Base(w.Dir)
		if w.Dir == "" {
			repoName = "(none)"
		}
		w.log = slog.With("repo", repoName)
	})
}

func (w *RepoWorkspace) reserveBranch(t *Task) {
	if w.Dir == "" {
		return
	}
	w.branchMu.Lock()
...
func (w *RepoWorkspace) allocateBranchLocked(ctx context.Context, t *Task) (string, error) {
...
func (w *RepoWorkspace) fetchAndCreateBranch(ctx context.Context, t *Task, branch string) error {
	w.branchMu.Lock()
	defer w.branchMu.Unlock()
...
func (w *RepoWorkspace) effectiveBaseBranch(t *Task) string {
