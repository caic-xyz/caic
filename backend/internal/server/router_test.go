...
func newRouterTestWorkspace(t testing.TB, baseBranch, dir string) *repowork.RepoWorkspace {
	workspace, err := repowork.NewRepoWorkspace(baseBranch, dir, filepath.Base(dir), time.Minute, nil, slog.With("repo", "test"))
	if err != nil {
		t.Fatal(err)
	}
	return workspace
