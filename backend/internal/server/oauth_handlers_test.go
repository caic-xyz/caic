...
func TestOAuthServer(t *testing.T) {
	...
	t.Run("token lifecycle", func(t *testing.T) {
		t.Run("refresh token and grant list survive server restart", func(t *testing.T) {
			...
			restarted := newTestRouterWithAuthHost(t, s.authStore, s.oauthRefreshTokenPath, auth.NewHostState("https://caic.example.com", nil))
	...
	t.Run("consent page", func(t *testing.T) {
		...
		t.Run("renders consent page with user info and scopes", func(t *testing.T) {
			...
			body := w.Body.String()
			// The tag-shaped assertions below match attribute fragments across line
			// breaks, so compare against whitespace-collapsed markup: how the
			// template happens to be formatted must not decide whether a control
			// is present.
			...
			if user2.AvatarURL != "" && strings.Contains(flat, user2.AvatarURL) {
...
func newMCPOAuthLifecycleRouter(t *testing.T, auditLogPath ...string) (*testRouter, http.Handler, auth.User, oauth.RegisterResponse) {
	...
	registerBody := `{"client_name":"Claude","redirect_uris":["https://claude.ai/api/mcp/auth_callback"],"token_endpoint_auth_method":"none","grant_types":["authorization_code","refresh_token"]}`
