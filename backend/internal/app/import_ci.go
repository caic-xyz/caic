...
func (w *importedTaskWiring) WireCIMonitoring(ctx context.Context, entry *taskmgr.Entry, checkout *repo.Checkout) {
	...
	if pr > 0 {
		...
		if err != nil {
			slog.InfoContext(ctx, "import: skipping CI monitor; PR head SHA unavailable", "task", t.ID, "branch", primary.Branch, "repo", primary.Name, "err", err)
			return
		}
		slog.InfoContext(ctx, "import: starting monitorCI", "task", t.ID, "branch", primary.Branch, "sha", sha)
...
func (w *importedTaskWiring) LookupExternalPRForTask(ctx context.Context, entry *taskmgr.Entry, checkout *repo.Checkout) {
	...
	slog.InfoContext(ctx, "import: found external PR", "repo", primary.Name, "br", primary.Branch, "pr", pr.Number)
