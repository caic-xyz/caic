// Tests for Store log segment creation, trailers, validated reopening, compression, and settled-task selection.
...
func TestStore(t *testing.T) {
	...
	t.Run("Reopen", func(t *testing.T) {
		...
		w, err := reopenTaskLog(&Store{LogDir: dir}, tk, path)
		if err != nil {
			t.Fatal(err)
		}
		if err := w.Close(); err != nil {
	...
	t.Run("CompressTerminalLogs", func(t *testing.T) {
		t.Parallel()
		t.Run("CompressesPurged", func(t *testing.T) {
			...
			if err := (&Store{LogDir: dir}).CompressTerminalLogs(logs); err != nil {
				t.Fatal(err)
			}
			...
			reloaded, err := storeFor(dir).Load()
			if err != nil {
				t.Fatal(err)
			}
			if len(reloaded) != 1 {
		...
		t.Run("SkipsStopped", func(t *testing.T) {
			...
			if err := (&Store{LogDir: filepath.Dir(path)}).CompressTerminalLogs(logs); err != nil {
				t.Fatal(err)
			}
...
func storeFor(logDir string) *Store { return &Store{LogDir: logDir} }
